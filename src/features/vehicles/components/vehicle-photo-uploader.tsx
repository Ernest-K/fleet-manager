...
import { useUploadVehiclePhoto } from "@/features/vehicles/hooks/useUploadVehiclePhoto";
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="flex justify-center aspect-square">
            <Button variant="outline" className="flex items-center grow h-full" onClick={() => document.getElementById("file-upload")?.click()}>
              {uploading ? (
                <Icons.spinner className="text-muted-foreground w-8 h-8 sm:w-12 sm:h-12 animate-spin" />
              ) : (
                <Plus className="text-muted-foreground w-8 h-8 sm:w-12 sm:h-12" />
              )}
            </Button>
