            ...
            <TableRow key={inspection.uid}>
              <TableCell className="flex items-center">
                {inspection.createdByUser?.role === Role.Driver ? (
                  <Link className={`${buttonVariants({ variant: "link" })} px-0 space-x-3`} href={`/dashboard/drivers/${inspection.createdByUser?.uid}`}>
                    <Avatar>
              ...
              <TableCell>{inspection.vehicle && <Link className={`${buttonVariants({ variant: "link" })} px-0`} href={`/dashboard/vehicles/${inspection.vehicle?.uid}`}>{`${inspection.vehicle?.make} ${inspection.vehicle?.model}`}</Link>}</TableCell>
