...
function InspectionDialog({ vehicleUid, inspection, trigger }: ServiceDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-2xl overflow-y-scroll max-h-screen no-scrollbar">
        ...
        <InspectionForm vehicleUid={vehicleUid} inspection={inspection} onSubmit={() => setOpen(false)} onCancel={() => setOpen(false)} />
      </DialogContent>
    </Dialog>
  );
}
